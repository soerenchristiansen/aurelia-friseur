...
export class AuthenticationService {
	...
	login(userName: string, password: string): Promise<object> {
		...
		return new Promise((resolve, reject) => {
			let body = `grant_type=password&client_id=${oidcConfiguration.client_id}&scope=${oidcConfiguration.scope}&username=${userName}&password=${password}`;

			let headers = {
				'Content-Type': 'application/x-www-form-urlencoded'
			};

			this.http.fetch(oidcConfiguration.token_endpoint, {
				method: 'post',
				body: body,
				headers: headers
			})
	...
	refreshToken(): Promise<object> {
		return new Promise((resolve, reject) => {
			...
			let body = `grant_type=refresh_token&client_id=${oidcConfiguration.client_id}&scope=${oidcConfiguration.scope}&refresh_token=${refreshToken}`;
	...
	getUser(): Promise<User> {
		...
		if (this.hasValidAccessToken()) {
			...
			return this.http.fetch(oidcConfiguration.userinfo_endpoint, {
				headers: {
